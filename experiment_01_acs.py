...
los_gr = sim_res[(sim_res.ID >= 0) | (sim_res.STATE.str.contains('E'))].groupby(['ID', 'STATE'])['TIME']
los = np.array((los_gr.max() - los_gr.min()).sum(level=0), dtype=float)
sns.distplot(los / 60.0)
plt.savefig('pics\\los-E-' + datetime.datetime.now().strftime('%Y-%m-%d-%H-%M-%S') + '.png')
plt.close()
# saving stats
