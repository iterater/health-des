...
states_pools = [
    state_info.load_state_pool(
        basic_path + "Transition_matrix" + str(i) + ".csv", basic_path + "Distr_states_" + str(i)
    )
...
acs_event_gen = state_info.PatientsDayFlowGenerator(
    basic_path + "Number_of_patients.txt", basic_path + "Entrance_time.txt"
)
    ...
    for tg_s in [0.5, 1.0, 1.5, 2.0]:
        for bg_s in [0.5, 1.0, 1.5, 2.0]:
            for nps in [1, 2, 3, 4, 5]:
                run_res = joblib.Parallel(n_jobs=6)(
                    joblib.delayed(single_experiment_run)(tg_s, bg_s, nps, True, i_run)
