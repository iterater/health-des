...
def patient(
    env, patient_id, pat_class_id, starting_state, states_pool, surgery_resource, log_track
):
    ...
    log_track.append(
        {
            ...
            "STATE": "IXX",
            "DIRECTION": "OUT",
            "QUEUE_TIME": env.now - time_before_queue,
            "QUEUE_LENGTH": 0,
        }
    )
...
def background_emitter(
    ...
    surgery_resource,
    log_track,
    surgery_bg_event_generator,
    surgery_bg_time_generator,
    surgery_bg_scale,
):
    ...
    while True:
        ...
        for i in range(1, len(seq) - 1):
            ...
            env.process(
                background_surgery_process(
                    env, surgery_resource, int(surgery_bg_time_generator.rvs()), log_track
                )
            )
...
def target_emitter(
    env,
    target_event_generator,
    target_patient_generator,
    surgery_resource,
    log_track,
    target_scale=1.0,
):
    ...
    while True:
        ...
        for i in range(1, len(seq) - 1):
            ...
            env.process(
                patient(env, counter, pat_class, pat_state, pat_pool, surgery_resource, log_track)
            )
            counter += 1
